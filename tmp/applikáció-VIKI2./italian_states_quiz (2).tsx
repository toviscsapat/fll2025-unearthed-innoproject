...
const ItalianStatesQuiz = () => {
  ...
  const getCorrectAnswer = () => {
    if (number % 2 === 0) {
      // Páros -> "P" betűvel kezdődik
      return answers.find(a => a.startsWith === "P");
    } else if (number % 3 === 0) {
      // Hárommal osztható -> négy szóból áll
      return answers.find(a => a.wordCount === 4);
    } else if (number % 5 === 0) {
      // Öttel osztható -> "Sz" betűvel kezdődik
      return answers.find(a => a.startsWith === "Sz");
    } else {
      // Különben -> "L" betűvel kezdődik
      return answers.find(a => a.startsWith === "L");
    }
  };
  ...
  const getAnswerOptions = () => {
    ...
    // Kiválasztjuk az egyes típusokat
    const pAnswer = answers.find(a => a.startsWith === "P" && a.id !== correctAnswer.id);
    const lAnswer = answers.find(a => a.startsWith === "L" && a.id !== correctAnswer.id);
    const fourWordAnswer = answers.find(a => a.wordCount === 4 && a.id !== correctAnswer.id);
    const szAnswer = answers.find(a => a.startsWith === "Sz" && a.id !== correctAnswer.id);
    
    // Összerakjuk az 5 választ: helyes + 4 különböző típus
    const allOptions = [
      correctAnswer,
      pAnswer,
      lAnswer,
      fourWordAnswer,
      szAnswer
    ].filter(a => a); // Csak azok maradnak, amik léteznek
    
    // Ha kevesebb mint 5, akkor feltöltjük másokkal
    if (allOptions.length < 5) {
      const remaining = answers.filter(a => !allOptions.includes(a));
  ...
  const [answerOptions, setAnswerOptions] = useState(getAnswerOptions());
        ...
        <div className="bg-white rounded-2xl shadow-xl p-6 md:p-8 border-t-4 border-blue-600">
          ...
          <div className="bg-gradient-to-r from-green-50 to-blue-50 rounded-xl p-6 mb-6 border-2 border-green-300">
            <h2 className="text-xl md:text-2xl font-bold text-gray-800">
              A(z) <span className="text-blue-700 text-3xl font-black">{number}</span> db olasz államból melyiknek volt a vezetője II. Viktor Emánuel?
            </h2>
          </div>
          ...
          <div className="bg-yellow-50 rounded-xl p-4 md:p-6 mb-6 border-2 border-yellow-200">
            ...
            <ul className="space-y-2 text-sm md:text-base text-gray-700">
              <li>✓ Ha a szám <strong>páros</strong>, akkor a válasz <strong>"P"</strong> betűvel kezdődik</li>
              <li>✓ Különben ha <strong>hárommal osztható</strong>, a válasz <strong>négy szóból</strong> áll</li>
              <li>✓ Különben ha <strong>öttel osztható</strong>, a válasz <strong>"Sz"</strong> betűvel kezdődik</li>
              <li>✓ Különben <strong>"L"</strong> betűvel kezdődik</li>
            </ul>
          </div>
            ...
            {answerOptions.map((answer) => {
              ...
              return (
                <div
                  ...
                  className={`${containerClass} rounded-xl p-4 cursor-pointer 
                    transition-all duration-200 hover:shadow-md flex items-center gap-4`}
                >
                  <div className={`w-6 h-6 rounded-full border-2 ${radioClass} 
          ...
          <div className="flex gap-3 flex-col sm:flex-row">
            {!showResult ? (
              ...
              <button
                onClick={reset}
                className="flex-1 bg-gray-600 text-white py-4 rounded-xl font-bold text-lg
          ...
          <div className="mt-6 p-4 bg-gray-50 rounded-lg border border-gray-200">
            <p className="text-sm text-gray-600">
              <strong>💡 Történelmi tény:</strong> II. Viktor Emánuel a Szardínia-Piemont Királyság uralkodója volt (1849-1861), 
              majd Olaszország első királya lett (1861-1878) az egyesítés után.
            </p>
          </div>
